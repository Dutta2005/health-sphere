...
import { GraduationCap, Stethoscope, Shield, Mail, Phone, MapPin, Ruler, Weight, Droplet, Info } from "lucide-react";
...
const UserIcon = ({status, ...props}: {status: string, className?: string}) => {
    if (status === "doctor") 
        return (
...
const DetailRow = ({ icon: Icon, label, value }: { icon: any, label: string, value: string | number }) => (
    <div className="flex items-center space-x-4 p-3 rounded-lg hover:bg-light-bg dark:hover:bg-dark-bg/50 transition-colors">
        ...
        <div className="flex-1">
            <p className="text-sm text-light-text/70 dark:text-dark-text/70">{label}</p>
        ...
        <Card className="overflow-hidden border-none shadow-lg">
                        ...
                        <div className="flex lg:flex-col items-center justify-between gap-2">
                            ...
                            {details.canDonate && (
                                <div className="inline-flex items-center gap-2 bg-accent/10 text-accent px-4 py-2 rounded-full">
                                    ...
                                    <span className="text-sm font-medium">Eligible Blood Donor</span>
                                </div>
                            )}
                        </div>
                        
                        <div className="grid gap-2">
                            <DetailRow icon={Mail} label="Email Address" value={details.email} />
                            <DetailRow icon={Phone} label="Phone Number" value={details.phone} />
                            <DetailRow icon={Droplet} label="Blood Group" value={details.info.bloodGroup} />
                            
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                                <DetailRow icon={Ruler} label="Height" value={`${details.info.height} cm`} />
                                <DetailRow icon={Weight} label="Weight" value={`${details.info.weight} kg`} />
                            </div>
            ...
            <CardFooter className="bg-light-bg dark:bg-dark-bg border-t p-4">
                <div className="flex items-center gap-2 text-xs text-light-text/70 dark:text-dark-text/70">
                    <Info size={15} />
                    <p>Your details are never shared with anyone.</p>
                </div>
            </CardFooter>
        </Card>
    );
